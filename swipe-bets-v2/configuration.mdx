...
const response = await getSwipeBets({
  disciplinesIds: ['football', 'basketball, dota'],  // Football and Basketball
  excludeIds: ['_id1'],     // Skip this recommendation
  pageNumber: 1,
  pageSize: 100
});
```
...
LS.set('swipeBetsExcludedIds', JSON.stringify(['_id1', '_id2']));
...
const queryString = (() => {
  ...
  params.append('outcomeIds[]', bet.id);
  params.append('currency', mobx.profile.currency.name);
